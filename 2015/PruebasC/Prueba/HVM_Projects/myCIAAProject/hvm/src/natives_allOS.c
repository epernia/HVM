...
#if defined(N_JAVA_LANG_CLASS_GETCONSTRUCTOR) || defined(N_JAVA_SECURITY_ACCESSCONTROLLER_DOPRIVILEGED) || defined(ENABLE_DEBUG) || defined(N_JAVA_LANG_CLASS_NEWINSTANCE) || defined(N_JAVA_LANG_CLASS_GETMETHOD)
...
extern void initializeException(int32* sp, int16 exceptionClass, int16 exceptionInitMethod);
 ...
 * return: int
 */
#ifdef N_TEST_ICECAPVM_MINITESTS_TESTINVOKENATIVE1_SUPER_TESTNATIVE
int16 n_test_icecapvm_minitests_TestInvokeNative1_Super_testNative(int32 *sp) {
    sp[0] = 42;
    return -1;
}
#endif
 ...
 * return: int
 */
#ifdef N_TEST_ICECAPVM_MINITESTS_TESTINVOKENATIVE1_SUPER_TESTNATIVESTATIC
int16 n_test_icecapvm_minitests_TestInvokeNative1_Super_testNativeStatic(int32 *sp) {
    sp[0] = 42;
    return -1;
}
#endif
 ...
 * return: int
 */
#ifdef N_TEST_ICECAPVM_MINITESTS_TESTINVOKENATIVE2_SUPER_TESTNATIVESTATIC
int16 n_test_icecapvm_minitests_TestInvokeNative2_Super_testNativeStatic(int32 *sp) {
    sp[0] = 42;
    return -1;
}
#endif
...
static int16 newInstance(int32* sp, unsigned short classIndex) {
    if (handleNewClassIndex(sp, classIndex)) {
        ...
        for (i = 0; i < NUMBEROFMETHODS; i++) {
            ...
            if (methodInfo->classIndex == classIndex) {
...
#if defined(N_JAVAX_SCJ_UTIL_VMFCT_CREATENATIVETASK) || defined(N_JAVA_LANG_THREAD_START)
unsigned char findMethodVTableIndex(unsigned short indexbyte1, unsigned char indexbyte2, unsigned short classIndex);

void* dispatchRunnable(void *arg) {
    Object* runnable = (Object*) arg;
    int32 *isrMethodStack;
    unsigned short clIndex = getClassIndex(runnable);
    unsigned short methodVtableIndex;
    unsigned short* vtable;

    methodVtableIndex = findMethodVTableIndex(JAVA_LANG_RUNNABLE, 0, clIndex);

    vtable = (unsigned short*) pgm_read_pointer(&classes[clIndex].vtable, unsigned short**);

    isrMethodStack = (int32 *)gc_allocateObject(50 * sizeof(int32), 0);
    *isrMethodStack = (int32)(pointer)runnable;
    enterMethodInterpreter(pgm_read_word(&vtable[methodVtableIndex]), isrMethodStack);
    return 0;
}
#endif

#if defined(N_TEST_ICECAPVM_MINITESTS_TESTCVAR1_GETIVAR)
/* getCVar
 * param :
 * return: int
 */
...
int16 n_test_icecapvm_minitests_TestCVar1_getIVar(int32 *sp)
{
    sp[0] = ivar;
    return -1;
}
...
#if defined(N_TEST_ICECAPVM_MINITESTS_TESTCVAR1_GETBVAR)
/* getCVar
 * param :
 * return: int
 */
...
int16 n_test_icecapvm_minitests_TestCVar1_getBVar(int32 *sp)
{
    sp[0] = bvar;
    return -1;
}
...
#if defined(N_TEST_ICECAPVM_MINITESTS_TESTCVAR1_GETSVAR)
/* getCVar
 * param :
 * return: int
 */
...
int16 n_test_icecapvm_minitests_TestCVar1_getSVar(int32 *sp)
{
    sp[0] = svar;
    return -1;
}
...
#if defined(N_TEST_ICECAPVM_MINITESTS_TESTCVAR1_GETLVAR)
/* getCVar
 * param :
 * return: int
 */
...
int16 n_test_icecapvm_minitests_TestCVar1_getLVar(int32 *sp)
{
    sp[1] = lvar;
    sp[0] = lvar >> 32;
    return -1;
...
int16 n_vm_Monitor_getAttachedMonitor(int32 *sp) {
    uint32* ptr;
    Object* monitor;
    ptr = (uint32*) ((unsigned char*) HEAP_REF((Object*) (pointer) sp[0], unsigned char*) - 4);
...
extern int16 vm_Monitor_notify(int32 *fp);
#endif
int16 n_java_lang_Object_notify(int32 *sp) {
#if defined(VM_MONITOR_NOTIFY_USED)
    return vm_Monitor_notify(sp);
#else
    return -1;
#endif
}
...
extern int16 vm_Monitor_wait(int32 *fp);
#endif
int16 n_java_lang_Object_wait_(int32 *sp) {
#if defined(VM_MONITOR_WAIT_USED)
    return vm_Monitor_wait(sp);
#else
    return -1;
#endif
}
...
#if defined(N_TEST_ICECAPVM_MINITESTS_TESTINVOKESTATIC2_APIDEMO1)
int16 n_test_icecapvm_minitests_TestInvokeStatic2_ApiDemo1(int32 *sp) {
    return -1;
}
#endif

#if defined(VM_CLOCKINTERRUPTHANDLER_INITIALIZE) || defined(VM_PROCESS_INIT__USED) || defined(VM_REALTIMECLOCK_GETREALTIMECLOCK_USED) || defined( REFLECT_CLASSINFO_GETCLASSINFOFROMARCHITECTURE) || defined(REFLECT_STATICREFINFO_GETOFFSETS)
#define X86_64 1
#define X86_32 2
#define CR16_C 3
#define ATMEGA2560 4
...
int16 n_java_lang_Class_getMethod(int32 *sp) {
    ...
    while (1) {
        ...
        if (classes[classIndex].superClass == -1) {
            initializeException(sp, JAVA_LANG_NOSUCHMETHODEXCEPTION, JAVA_LANG_NOSUCHMETHODEXCEPTION_INIT_);
...
int16 n_java_lang_reflect_Constructor_newInstance(int32 *sp) {
    ...
    classIndex = *(unsigned short *) ((unsigned char*) HEAP_REF(constructor, Object*) + sizeof(Object) + sizeof(unsigned short));
    handleNewClassIndex(sp, classIndex);
    sp[2] = (int32) (pointer) args;
    sp[1] = sp[0];
    sp[0] = (int32) (pointer) constructor;
