...
final class ScjProcess implements Comparable<ScjProcess> {
	...
	int index = -9999; // The index of the ScjProcesses; used by
	...
	ScjProcess(ManagedEventHandler handler, int[] stack) {
		...
		this.process = new vm.Process(new ProcessLogic() {
			...
			public void catchError(final Throwable t) {
				...
				try {
					ImmortalMemory.instance().executeInArea(exceptionReporter);
		...
		if (target instanceof PeriodicEventHandler) {
			...
			// HSO: Dec 2013: RelativeTime in this version; later perhaps
			// AbsoluteTime too
	...
	public int compareTo(ScjProcess process) {
		// HSO: original: two queues in PriorityFrame: PriorityQueue and
		// SleepingQueue (works)
		return (this.target.priority.getPriority() - process.target.priority
				.getPriority());

		// APR: proposal: one queue in PriorityFrame: ReleaseQueue (does not
		// work)
		// int result= this.next.compareTo(process.next);
		// if (result == 0)
		// {
		// result = (process.target.priority.getPriority() -
		// this.target.priority.getPriority());
		// //result = (this.target.priority.getPriority()
		// -process.target.priority.getPriority());
		// }
		// return result;

		// HSO: tried to test priority at first; does not work
		// int result = (process.target.priority.getPriority() -
		// this.target.priority.getPriority());
		// if (result == 0)
		// {
		// result = this.next.compareTo(process.next);
		//
		// }
		// return result;
	}
	...
	public static ScjProcess createIdleProcess() {
		if (idleProcess == null) {
			idleProcess = new ScjProcess(new PeriodicEventHandler(
