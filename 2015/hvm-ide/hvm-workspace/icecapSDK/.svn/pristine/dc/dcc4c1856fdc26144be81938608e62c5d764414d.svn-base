...
public abstract class ManagedEventHandler implements ManagedSchedulable {
	...
	public ManagedEventHandler(PriorityParameters priority,
		...
		this.storage = (storage != null ? storage : getDefaultStorage());

		this.privateMemory = new PrivateMemory(
				(storage != null ? (int) storage.maxMemoryArea
						: Const.PRIVATE_MEM_SIZE_DEFAULT), "PvtMem");
	}

	private static StorageParameters getDefaultStorage() {
		if (defaultStorage == null) {
			defaultStorage = new StorageParameters(0,
					new long[] { Const.HANDLER_STACK_SIZE_DEFAULT }, 0, 0, 0);
		}
