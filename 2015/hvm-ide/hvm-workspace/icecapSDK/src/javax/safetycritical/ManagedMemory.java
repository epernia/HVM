...
public abstract class ManagedMemory extends MemoryArea {
	...
	// APR currentMemory is dynamically located through the currently running
	// handler
	static final ManagedMemory getCurrentMemory() {
	...
	static void allocateBackingStore(int size) {
		 ...
		 * backingStoreBase + ";end: " + backingStoreEnd + "; size: " + size);
		 */
	}

	public static class ImmortalMemory extends ManagedMemory // HSO: not public
	{
		private static ImmortalMemory immortal;

		ImmortalMemory(int sizeOfArea) {
			super(sizeOfArea, null);
			immortal = this;
		}

		static ImmortalMemory instance() {
			return (ImmortalMemory) immortal;
		}
	}
	 ...
	 */
	ManagedMemory(int size, String label) {
		super(size, label);
	}
	...
	static final ManagedMemory getOuterMemory(ManagedMemory m) {
		if (m instanceof InnerPrivateMemory)
			return ((InnerPrivateMemory) m).prev;
		if (m instanceof PrivateMemory)
			return Mission.currMissSeq.missionMemory;
		if (m instanceof MissionMemory)
			return ImmortalMemory.instance();
		// since level 0 and 1 have no nested missions, the outer is immortal
		// and stays there.
		return ImmortalMemory.instance();
	}
	...
	public static void enterPrivateMemory(int size, Runnable logic)
		...
		prev.delegate.switchToArea(inner.delegate);
		logic.run();
		inner.delegate.switchToArea(prev.delegate);
	...
	public static void executeInAreaOf(Object obj, Runnable logic) {
		...
		if (prev == null) {
			...
			currentMem = prev.delegate;
		}

		currentMem.switchToArea(target.delegate);
		logic.run();
		target.delegate.switchToArea(currentMem);
