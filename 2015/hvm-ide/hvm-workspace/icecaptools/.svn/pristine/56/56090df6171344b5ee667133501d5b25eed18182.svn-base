...
public class CallGraph {
    ...
    public void register(BNode cause, String locationClass, String locationMethod, String locationMethodSignature, String targetClassName, String targetMethodName, String targetMethodSignature) {
        MethodMap<CGInfo> callees;

        if (!cg.contains(locationClass, locationMethod, locationMethodSignature)) {
            callees = new MethodMap<CGInfo>();
            cg.put(locationClass, locationMethod, locationMethodSignature, callees);
        } else {
            callees = cg.get(locationClass, locationMethod, locationMethodSignature);
        }

        CGInfo cgInfo;
        if (!callees.contains(targetClassName, targetMethodName, targetMethodSignature)) {
            ...
            cgInfo = callees.get(targetClassName, targetMethodName, targetMethodSignature);
        }
                ...
                if (nextMethod.canCallWithArgs()) {
                    ...
                    if (callSites != null) {
                        ...
                        while (callSitesIterator.hasNext() && (carryOn == false)) {
                            ...
                            Iterator<String> targetClasses = callsite.getTargetClasses();
                            while (targetClasses.hasNext() && (carryOn == false)) {
                                String nextClass = targetClasses.next();
    ...
    private static class EmptyMethodIdentifierIterator implements Iterator<MethodIdentifier> {
    ...
    private static class MethodIdentifierIterator implements Iterator<MethodIdentifier> {
    ...
    public Iterator<MethodIdentifier> getCallees(String className, String methodName, String methodSignature) {
