...
public class TestSCJBoundedBuffer {
    ...
    private static class Producer extends PeriodicEventHandler {
        ...
        public Producer(PriorityParameters priority, 
                PeriodicParameters periodic, 
                StorageParameters storageParameters,
                BoundedBuffer buf, 
                AperiodicEventHandler aevh, AperiodicEventHandler aevh1) {
    ...
    private static class Consumer extends PeriodicEventHandler {
        ...
        public Consumer(PriorityParameters priority, 
                PeriodicParameters periodic, 
                StorageParameters storageParameters,
    ...
    private static class Display extends PeriodicEventHandler {
        ...
        public Display(PriorityParameters priority, 
                PeriodicParameters periodic, 
                StorageParameters storageParameters,
                BoundedBuffer buf, AperiodicEventHandler aevh) {
            super(priority, periodic, storageParameters);
    ...
    private static class BoundedBuffer {
        ...
        public synchronized boolean invariant() {
            return 0 <= count && count <= size && 0 <= rear && rear < size 
                   && 0 <= front && front < size && rear == (front + count) % size;
        }
    }
    ...
    private static class MyMission extends Mission {
        ...
        public void initialize() {
            ...
            new Producer(new PriorityParameters(Priorities.PR97), 
                         new PeriodicParameters(new RelativeTime(Clock.getRealtimeClock()), // start
                         new RelativeTime(100, 0, Clock.getRealtimeClock())), // period
                         storageParameters,
                         buffer, aevh, aevh1).register();

            new Consumer(new PriorityParameters(Priorities.PR96), 
                    new PeriodicParameters(new RelativeTime(Clock.getRealtimeClock()), // start
                    new RelativeTime(200, 0, Clock.getRealtimeClock())), // period
                    storageParameters,
                    buffer, aevh).register();

            new Display(new PriorityParameters(Priorities.PR95), 
                    new PeriodicParameters(new RelativeTime(Clock.getRealtimeClock()), // start
    ...
    private static class MyApp implements Safelet<MyMission> {
        ...
        private class MySequencer extends MissionSequencer<MyMission> {
            ...
            MySequencer() {
                super(new PriorityParameters(Priorities.PR94), storageParameters); // mission
