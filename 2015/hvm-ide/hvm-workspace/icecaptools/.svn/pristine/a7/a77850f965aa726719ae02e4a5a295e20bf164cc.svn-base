...
public abstract class AOTCompiler implements SPManipulator {
    ...
    public String compile() throws Exception {
        ...
        MethodEntryPoints entrypoints = toolBox.getDependencyExtent().getMethod(currentClassName, methodName, methodSignature);
        ...
        StackManager sm = new LevelNRegisterAllocator(body, localVariables, javaMethod.getCode().getMaxStack(), javaMethod.getCode().getMaxLocals(), javaMethod.isSynthetic(), this);
        ...
        if (entrypoints == null) {
            ...
            fullFunction.append("   return -1;\n");
        } else {
            TargetAddressMap tMap = entrypoints.getJumpTargets();

            StringBuffer exceptionSwitch = new StringBuffer();
            LabelsManager labelsManager = new LabelsManager(entrypoints, methodNumber, localVariables, requiredIncludes, this);
    ...
    public void compileByteCode(StackManager sm, LabelsManager labelsManager, TargetAddressMap tMap, StringBuffer output, NoDuplicatesMemorySegment localVariables, int pc, boolean compileOne, MethodEntryPoints entrypoints) throws Exception {
        while (pc < currentMethodCode.length) {
            bnode = toolBox.getDependencyExtent().getBNodeFromMethod(toolBox.getCurrentClassName(), javaMethod.getName(), javaMethod.getSignature(), pc);
            toolBox.getTool().getSourceCodeLinker().getSourceCode(toolBox.getCurrentClassName(), javaMethod, bnode.getOriginalAddress(), output);
            ...
            case RawByteCodes.areturn_opcode: {
                ...
                if (synchronizedSupported(javaMethod)) {
                    if (entrypoints.canCallWithArgs()) {
                        NoDuplicatesMemorySegment thisName = new NoDuplicatesMemorySegment(toolBox.getTool().getProperties());
            ...
            case RawByteCodes.iinc_opcode: {
                ...
                output.append("   " + sm.getLocal(localVarNo, bnode.getOriginalAddress()) + " = (" + cast + ")" + sm.getLocal(localVarNo, bnode.getOriginalAddress()) + " + " + constVal + ";\n");
                pc++;
                break;
            }
            ...
            case RawByteCodes.new_opcode: {
                ...
                output.append("   sp++;\n");
                pc += 3;
                requiredIncludes.print("extern unsigned char handleNewClassIndex(int32* sp, unsigned short classIndex);\n");
            ...
            case RawByteCodes.invokestatic_opcode: {
                ...
                AOTInvokeSpecialEmitter emitter = new AOTInvokeSpecialEmitter(sm, output, localVariables, pc, labelsManager, bnode, currentMethodCode, requiredIncludes, methodDesc, toolBox, entrypoints);
                ...
                requiredIncludes.print("extern int16 enterMethodInterpreter(unsigned short methodNumber, int32* sp);\n");
            ...
            case RawByteCodes.invokeinterface_opcode: {
                ...
                if (useDirectCall) {
                    ...
                    sm.flush(true);
                    adjustStackAndCheckObject(output, localVariables, pc, labelsManager, numArgs + 1, "      classIndex = getClassIndex(i_obj);\n", "   ", sm);
                    requiredIncludes.print("extern unsigned short getClassIndex(Object* obj);\n");
                    ...
                    handleExceptionOccurred(output, localVariables, pc, labelsManager, returnValueString, "   ", sm, null, exceptionVariable, entrypoints, exceptionVariable + " == -1", false);

                    requiredIncludes.print("extern signed short dispatchInterface(unsigned short methodIndex, unsigned char *vTableIndex, int32* sp);\n");
                }
                pc += bnode.getRawBytes().length;
                break;
            }
            ...
            case RawByteCodes.aload_3_opcode:
                ...
                sm.push(Size.INT, sm.getLocal(currentMethodCode[pc] - RawByteCodes.aload_0_opcode, getOriginalAddress(pc)));
                pc++;
                break;
            ...
            case RawByteCodes.astore_3_opcode: {
                ...
                if (bnode.isRedundant()) {
                    ...
                    dst = "   " + sm.getLocal(currentMethodCode[pc] - RawByteCodes.astore_0_opcode, getOriginalAddress(pc));
                }
            ...
            case RawByteCodes.iload_3_opcode: {
                ...
                if ((currentMethodCode[pc] == RawByteCodes.iload_opcode) || (currentMethodCode[pc] == RawByteCodes.fload_opcode)) {
                    pc++;
                }
                pc++;
                break;
            ...
            case RawByteCodes.putfield_opcode: {
                ...
                if (currentMethodCode[pc] == RawByteCodes.putfield_opcode) {
                    ...
                    int referenceAddressIndex = toolBox.getPatcher().getClassNumber(FieldOffsetCalculator.ReferenceAddressClassName);
                    ...
                    case 64:
                        ...
                        requiredIncludes.print("extern void writeLongToIO(pointer address, unsigned short offset, int32 msb, int32 lsb);\n");
                        break;
                    case 32:
                        output.append("      writeIntToIO((pointer)hw_address, " + foffset + ", lsb_" + type + ");\n");
                        requiredIncludes.print("extern void writeIntToIO(pointer address, unsigned short offset, int32 lsb);\n");
                        break;
                    case 16:
                        output.append("      writeShortToIO((pointer)hw_address, " + foffset + ", lsb_" + type + " & 0xffff);\n");
                        requiredIncludes.print("extern void writeShortToIO(pointer address, unsigned short offset, unsigned short lsb);\n");
                        break;
                    case 8:
                        output.append("      writeByteToIO((pointer)hw_address, " + foffset + ", lsb_" + type + " & 0xff);\n");
                        requiredIncludes.print("extern void writeByteToIO(pointer address, unsigned short offset, unsigned char lsb);\n");
                        break;
                    case 1:
                        output.append("      writeBitToIO((pointer)hw_address, " + foffset + ", lsb_" + type + " & 0x1);\n");
                        requiredIncludes.print("extern void writeBitToIO(pointer address, unsigned short offset, unsigned char bit);\n");
                        break;
                    }
                }
                pc += 6;
            ...
            case RawByteCodes.getfield_opcode: {
                ...
                if (currentMethodCode[pc] == RawByteCodes.getfield_opcode) {
                    ...
                    case 64:
                        ...
                        requiredIncludes.print("extern void readLongFromIO(pointer address, unsigned short offset, int32* msb, int32* lsb);\n");
                        break;
                    case 32:
                    ...
                    case 16:
                        ...
                        requiredIncludes.print("extern int16 readShortFromIO(pointer address, unsigned short offset);\n");
                        break;
                    case 8:
            ...
            case RawByteCodes.newarray_opcode: {
                ...
                output.append("      narray = (Object*) createArray(" + classIndex + ", (uint16) count FLASHARG((0)));\n");
                ...
                pc += 3;
                requiredIncludes.print("extern unsigned char* createArray(unsigned short classIndex, uint16 count FLASHARG(uint8 flash));\n");
                requiredIncludes.print("extern unsigned short getClassIndex(Object* obj);\n");
                break;
            }
            case RawByteCodes.multianewarray_opcode: {
                ...
                sm.flush(false);
                localVariables.print("   Object* narray;\n");
                output.append("      narray = (Object*) createMultiDimensionalArrays(sp, " + dimensions + ", " + classIndex + ");\n");
                while (dimensions > 0) {
                ...
                sm.push(Size.INT, "(int32) (pointer) narray");
                pc += 4;
                requiredIncludes.print("extern Object* createMultiDimensionalArrays(int32* sp, unsigned char dimensions, unsigned short classIndex);\n");
                break;
            }
            ...
            case RawByteCodes.iastore_opcode: {
                ...
                if ((currentMethodCode[pc] != RawByteCodes.lastore_opcode) && (currentMethodCode[pc] != RawByteCodes.dastore_opcode)) {
            ...
            case RawByteCodes.putstatic_opcode: {
                StaticFieldEmitter emitter = new AOTStaticFieldEmitter(pc, toolBox, currentMethodCode, localVariables, output, sm, requiredIncludes, bnode);
            ...
            case RawByteCodes.lstore_opcode: {
                short byteVal;
                int pcInc;
                if ((currentMethodCode[pc] == RawByteCodes.dstore_opcode) || (currentMethodCode[pc] == RawByteCodes.lstore_opcode)) {
    ...
    private int getOriginalAddress(int pc) {
        BNode bnode = toolBox.getDependencyExtent().getBNodeFromMethod(toolBox.getCurrentClassName(), javaMethod.getName(), javaMethod.getSignature(), pc);
    ...
    private int getProducerSize(BNode producer, int stackCellIndex) throws Exception {
        ...
        case RawByteCodes.invokestatic_opcode: {
            ...
            if (size <= Size.INT) {
                ...
                if ((producer.getAinfo().exitStack.size() == (stackCellIndex + 1)) || (producer.getAinfo().exitStack.size() == (stackCellIndex + 2))) {
                    return Size.INT;
                }
            }
            break;
    ...
    private void handleExceptionOccurred(StringBuffer output, NoDuplicatesMemorySegment localVariables, int pc, LabelsManager labelsManager, String returnValueString, String indent, StackManager sm, MethodOrFieldDesc methodDesc, String exceptionVariable,
    ...
    private void handleIt(StringBuffer output, NoDuplicatesMemorySegment localVariables, int pc, LabelsManager labelsManager, String indent, StackManager sm, String exceptionVariable, MethodEntryPoints entrypoints) throws Exception {
        ...
        if (hasExceptionHandlers(javaMethod)) {
            ...
            output.append(indent + "      return " + exceptionVariable + ";\n");
        }
    }
    ...
    protected boolean nullPointerCheckRequired(int pc, int numArgs) {
        BNode bnode = (BNode) toolBox.getDependencyExtent().getBNodeFromMethod(toolBox.getCurrentClassName(), javaMethod.getName(), javaMethod.getSignature(), pc);
    ...
    private class AOTInvokeSpecialEmitter extends InvokeSpecialEmitter {
        AOTInvokeSpecialEmitter(StackManager sm, StringBuffer output, NoDuplicatesMemorySegment localVariables, int pc, LabelsManager labelsManager, BNode bnode, byte[] currentMethodCode, NoDuplicatesMemorySegment requiredIncludes, MethodOrFieldDesc methodDesc,
                AOTToolBox toolBox, MethodEntryPoints entrypoints) {
        ...
        @Override
        protected void a_handleExceptionOccurred(StringBuffer output, NoDuplicatesMemorySegment localVariables, int pc, LabelsManager labelsManager, String returnValueString, String string, StackManager smCopy, MethodOrFieldDesc methodDesc, String exceptionVariable,
                MethodEntryPoints entrypoints, String noExceptionCondition, boolean negateExceptionValue) throws Exception {
            handleExceptionOccurred(output, localVariables, pc, labelsManager, returnValueString, string, smCopy, methodDesc, exceptionVariable, entrypoints, noExceptionCondition, negateExceptionValue);
        }
    }
}
