...
#if defined(TEST_ICECAPVM_MINITESTS_TESTHWOBJECT_MAIN)
    if (saddress == address) {
        if (soffset == offset) {
            *msb = smsb;
            *lsb = slsb;
...
#if defined(N_TEST_ICECAPVM_MINITESTS_TESTCAS_ATOMICREFERENCE_CAS)
int32 n_test_icecapvm_minitests_TestCAS_AtomicReference_cas(int32 *sp) {
...
#if defined(N_TEST_ICECAPVM_MINITESTS_TESTPERFORMANCE_QUICKSORTC)
...
uint16 n_test_icecapvm_minitests_TestPerformance_quicksortC(uint32 *sp)
{
