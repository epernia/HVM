...
public class DependencyWalker {
    ...
    private static class DependencyWalkerObserver implements AnalysisObserver {
        ...
        public void registerNativeField(String containingClass, int fieldOffset, FieldInfo field, IcecapCVar cvar) {
            delegate.registerNativeField(containingClass, fieldOffset, field, cvar);
        }

        @Override
        public NativeFieldInfo isNativeField(String containingClass, int foffset) {
            return delegate.isNativeField(containingClass, foffset);
        }
    ...
    private void analyseBNode(BNode bnode, NewList newList, NewList mr, Stack<WorkItem> workItemStack) throws Throwable {
        ...
        if (newList.lessThanOrEquals(bnodeNewList)) {
            ...
            while (exceptions.hasNext()) {
                ...
                JavaClass clazz = Repository.lookupClass(nextException);
            ...
            try {
                if (bnode instanceof BasicBNode) {
                    ...
                    analyseInterfaceMethodCallBNode((MethodCallBNode) bnode, newList, mr, workItemStack, implementorChecker);
    ...
    private void analyseObjectFieldAccessBNode(ObjectFieldAccessBNode bnode, NewList newList, NewList mr, Stack<WorkItem> workItemStack) throws Exception {
        JavaClass clazz = Repository.lookupClass(bnode.getClassName());
    ...
    private void analyseNewArrayBNode(NewArrayBNode bnode, NewList newList, NewList mr, Stack<WorkItem> workItemStack) throws Exception {
        ...
        if (JavaArrayClass.isArrayClass(type)) {
            ...
            if ((referredType.length() > 0) && JavaArrayClass.isReferenceClass(referredType)) {
                ...
                JavaClass elementClazz = Repository.lookupClass(elementType);
    ...
    private void analyseLDCBNode(LDCBNode bnode, NewList newList, NewList mr, Stack<WorkItem> workItemStack) throws Throwable {
        ...
        if (constant.getType() == LDCConstant.STRING) {
            ...
            JavaClass clazz = Repository.lookupClass(constant.getClassName());
    ...
    public NewList analyseMethod(MethodEntryPoints entryPoints, NewList newList) throws Throwable {
        ...
        Iterator<BNode> handlerEntryPoints = entryPoints.getHandlerEntryPoints();
    ...
    private void analyseVirtualMethodCallBNode(MethodCallBNode bnode, NewList newList, NewList mr, Stack<WorkItem> workItemStack, CheckSubClassRelationShip checker) throws Throwable {
        ...
        MethodAndClass methodAndClass = ClassfileUtils.findMethodInClassHierarchy(clazz, bnode.getMethodName(), bnode.getMethodSig());
    ...
    private void ensureInvocationTargetException(MethodCallBNode bnode, NewList newList, NewList mr) throws Throwable {
        JavaClass clazz = Repository.lookupClass("java/lang/reflect/InvocationTargetException");

        if (clazz != null) {
            MethodEntryPoints entryPoints = converter.convertByteCode(bnode, clazz, "<init>", "(Ljava/lang/Throwable;)V", false, true);
    ...
    private void handleDoPriviledge(MethodCallBNode bnode, NewList newList, NewList mr) throws Throwable {
        ...
        JavaClass clazz = Repository.lookupClass(nextClass);
    ...
    private void handleNewInstance(MethodCallBNode bnode, NewList newList, NewList mr) throws Throwable {
        Iterator<String> forcedincludes = config.getForcedIncludes();
    ...
    private void ensureUnboxing(MethodCallBNode bnode, NewList newList, NewList mr) throws Throwable {
        String[] methods = { "unbox", "(Ljava/lang/Object;)V", "boxBoolean", "(Z)Ljava/lang/Boolean;", "boxByte", "(B)Ljava/lang/Byte;", "boxShort", "(S)Ljava/lang/Short;", "boxCharacter", "(C)Ljava/lang/Character;", "boxInteger", "(I)Ljava/lang/Integer;", "boxLong",
                "(J)Ljava/lang/Long;",  };

        
        
        
        JavaClass clazz = Repository.lookupClass("reflect/Unboxing");
    ...
    private void analyseMethodCallBNode(JavaClass clazz, MethodCallBNode bnode, NewList newList, NewList mr, Stack<WorkItem> workItemStack, CheckSubClassRelationShip checker) throws Throwable {
        ...
        while (count < elements.length) {
            ...
            if (!JavaArrayClass.isArrayClass(className)) {
                JavaClass subClazz = Repository.lookupClass(className);

                if (checker.isSubclassOf(subClazz, clazz)) {
                    subClazz = ClassfileUtils.findMethodInClassHierarchy(subClazz, bnode.getMethodName(), bnode.getMethodSig()).getClazz();

                    bnode.addTarget(subClazz.getClassName());

                    addMethodToExtent(bnode, subClazz, bnode.getMethodName(), bnode.getMethodSig(), false, newList, exitList);
                }
            }
        }
    ...
    private void checkNatives(BNode cause, NewList newList, JavaClass clazz, String methodName, String methodSignature, NewList mr) throws Throwable {
        Method method = ClassfileUtils.findMethodInClass(clazz, methodName, methodSignature);
        if (method != null) {
            if (method.isNative() || converter.skipMethodHack(clazz.getClassName(), methodName, methodSignature) || hackHack(clazz, methodName, methodSignature)) {
                ...
                for (Type type : types) {
                    if (type instanceof ObjectType) {
                        ...
                        JavaClass runnable = Repository.lookupClass("java.lang.Runnable");
                        JavaClass argClass = Repository.lookupClass(typeName);
    ...
    private void analyseSpecialMethodCallBNode(MethodCallBNode bnode, NewList newList, boolean isStatic, NewList mr, Stack<WorkItem> workItemStack) throws Throwable {
        ...
        declaringClass = ClassfileUtils.findMethodInClassHierarchy(clazz, bnode.getMethodName(), bnode.getMethodSig()).getClazz();
        ...
        addMethodToExtent(bnode, declaringClass, bnode.getMethodName(), bnode.getMethodSig(), isStatic, newList, exitList);
                ...
                if (!isStatic) {
                    ...
                    if (interfaces != null) {
                        for (JavaClass iface : interfaces) {
                            ...
                            try {
                                JavaClass jmlSurrogateClass = Repository.lookupClass(surrogate);
    ...
    private void ensureJMLMethod(MethodCallBNode bnode, NewList newList, NewList exitList, JavaClass superClass, String jmlMethodName) throws Exception, Throwable {
        ...
        for (Method method : methods) {
            if (method.getName().contains(jmlMethodName)) {
                entryPoints = converter.convertByteCode(bnode, superClass, method.getName(), method.getSignature(), false);
                if (entryPoints != null) {
                    NewList calledMethodResult = analyseMethod(entryPoints, new NewList(newList));
                    exitList.merge(calledMethodResult);
                }
