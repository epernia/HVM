...
public class Converter implements ClassManager {
    ...
    public NewList startConversion(ConversionConfiguration config) {
        ...
        try {
            ...
            JavaClass clazz = Repository.lookupClass(config.getInputPackage() + "." + config.getInputClass());
    ...
    public MethodEntryPoints convertByteCode(BNode cause, JavaClass targetClass, String targetMethodName, String targetMethodSignature, boolean isStatic, boolean forceInclusion) throws Exception {
        ...
        registerInCallGraph(cause, targetClass.getClassName(), targetMethodName, targetMethodSignature);
    ...
    private void registerInCallGraph(BNode cause, String targetClassName, String targetMethodName, String targetMethodSignature) {
        if (cause != null) {
            cg.register(cause, cause.locationClass, cause.locationMethod, cause.locationMethodSignature, targetClassName, targetMethodName, targetMethodSignature);
        }
    }
            ...
            if (code != null) {
                ...
                while (stream.hasMore()) {
                    ...
                    if (current instanceof Raw_goto) {
                        ...
                        currentBNode = new StaticFieldAccessBNode(putgetstatic.getAddress(), className.getBytes(), fieldName, signature, current instanceof Raw_getstatic, clazz.getClassName(), targetMethodName, targetMethodSignature);
