...
void initializeException(int32* sp, int16 exceptionClass, int16 exceptionInitMethod) _NOINLINE_;
...
unsigned char handleMonitorEnterExit(Object* lockObj, unsigned char isEnter, int32* sp, const char* fromMethod);
...
static int32 methodInterpreter(unsigned short currentMethodNumber, int32* fp) {
    ...
    start: {
        ...
        method_code = (unsigned char *) pgm_read_pointer(&currentMethod->code, unsigned char**);
    ...
    loop: while (1) {
        ...
        case IRETURN_OPCODE: {
            ...
            const char* fromMethod = (const char*) pgm_read_pointer(&currentMethod->name, const char**);
            int32 spm1 = *(sp - 1);
            int32 spm2 = *(sp - 2);
            ...
            case CHECKCAST_OPCODE: {
                ...
                if (object != 0) {
                    ...
                    if (throwException && (code == CHECKCAST_OPCODE)) {
                        initializeException(sp, JAVA_LANG_CLASSCASTEXCEPTION_var, JAVA_LANG_CLASSCASTEXCEPTION_INIT__var);
                        sp++;
                        break;
                    }
                } else {
        ...
        initializeException(sp, JAVA_LANG_NULLPOINTEREXCEPTION_var, JAVA_LANG_NULLPOINTEREXCEPTION_INIT__var);
        sp++;
        goto throwIt;
        ...
        callMethod: {
            ...
            if (code != INVOKESTATIC_OPCODE) {
                if (minfo >> 7) {
                    ...
                    if (!handleMonitorEnterExit(this, 1, spm, (const char*) pgm_read_pointer(&currentMethod->name, const char**))) {
                        spm++;
                        sp = spm;
                        goto throwIt;
                    }
            ...
            if (pgm_read_pointer(&methodInfo->code, unsigned char**) == 0) {
                ...
                excep = nativeFunc(sp);
                if (excep == -1) {
                    unsigned char numReturnArgs = pgm_read_byte(&methodInfo->minfo) & 0x3;
...
#endif
void initializeException(int32* sp, int16 exceptionClass, int16 exceptionInitMethod) {
#if defined(PRE_INITIALIZE_EXCEPTIONS)
    unsigned short index = 0;
...
static unsigned short getFieldInfo(unsigned char* method_code, unsigned short *offset, unsigned char *size) {
    unsigned char fsize;
...
unsigned char handleMonitorEnterExit(Object* this, unsigned char isEnter, int32* sp, const char* fromMethod) {
    uint32* ptr;
    Object* monitor;
...
unsigned short getMainMethod(void) {
